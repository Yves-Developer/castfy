...
export function NewDemo() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      // A project exists before any footage does: this creates it, then the
      // studio's agent sidebar records into it.
      try {
        const project = await bridge.createProject(value.title);
        close();
        form.reset();
        navigate(`/demo/${project.id}`);
      } catch (error) {
        toast.error("Could not create the demo", {
