...
export function Settings({
  ...
  useEffect(() => {
    ...
    const tab = TABS.find((t) => t.value === defaultTab) ?? DEFAULT_TAB;
    setActiveTab(tab.value);
    setActivePage(tab.pages[0].value);
    // Only re-sync when the dialog opens or the requested tab changes
  }, [isOpen, defaultTab]);
  const handleTabChange = (value: string) => {
    const tab = TABS.find((t) => t.value === value);
    setActiveTab(value as "account" | "workspace");
