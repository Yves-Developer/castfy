...
function ensureBuf(ac: AudioContext): AudioBuffer {
  if (_buf && _buf.sampleRate === ac.sampleRate) {
    return _buf;
  }
...
function IcoPause({ size = 16 }: { size?: number }) {
  return (
    <svg fill="currentColor" height={size} viewBox="0 0 16 16" width={size}>
      <title>Pause</title>
      <rect height="12" rx="0.75" width="3.5" x="3" y="2" />
      <rect height="12" rx="0.75" width="3.5" x="9.5" y="2" />
    </svg>
  );
}
...
export function VideoPlayerPro({
  ...
  const syncTime = () => {
    ...
    setTime(Number.isFinite(t) ? t : 0);
    const p = d > 0 ? (t / d) * 100 : 0;
    setProg(Number.isFinite(p) ? p : 0);
  };
