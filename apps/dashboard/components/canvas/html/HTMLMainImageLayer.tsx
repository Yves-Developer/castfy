...
import { ChromeToolbar, SafariToolbar } from "../frames/BrowserToolbar";
...
interface HTMLMainImageLayerProps {
  ...
  setScreenshot: (
    updates: Partial<HTMLMainImageLayerProps["screenshot"]>
  ) => void;
...
function buildDropShadowFilter(shadow: ShadowConfig): string | undefined {
  ...
  if (colorMatch) {
    ...
    r = Number.parseInt(parts[0]) || 0;
    g = Number.parseInt(parts[1]) || 0;
    b = Number.parseInt(parts[2]) || 0;
...
export function HTMLMainImageLayer({
  ...
  const frameBoxShadow = useMemo(
    () => (isStyleFrame ? buildBoxShadow(shadow) : undefined),
    [shadow, isStyleFrame]
  );
  ...
  const isMacFrame =
    frame.type === "macos-light" || frame.type === "macos-dark";
  const isWinFrame =
    frame.type === "windows-light" || frame.type === "windows-dark";
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: PointerEvent) => {
      ...
      const newScale = Math.round(
        Math.min(200, Math.max(10, start.scale + scaleDelta))
      );
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: PointerEvent) => {
      ...
      const currentAngle =
        Math.atan2(e.clientY - start.centerY, e.clientX - start.centerX) *
        (180 / Math.PI);
  ...
  const getFrameContainerStyle = (): React.CSSProperties => {
    ...
    if (isStyleFrame) {
      const styleConfig: Record<string, { bg: string }> = {
        ...
        "outline-light": {
          bg: `rgba(255, 255, 255, ${frame.opacity ?? 0.35})`,
        },
      ...
      const outerRadius =
        screenshot.radius > 0 ? screenshot.radius + windowPadding : 0;
      return {
        ...baseStyle,
