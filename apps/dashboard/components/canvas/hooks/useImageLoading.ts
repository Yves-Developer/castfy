...
import { isOverlayPath } from "@/lib/r2/r2-overlays";
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => {
      ...
      if (retries < MAX_RETRIES) {
        ...
        reject(
          new Error(
            `Failed to load image after ${MAX_RETRIES + 1} attempts: ${url}`
          )
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const loadOverlays = async () => {
      ...
      const loadPromises = visibleOverlays.map((overlay) => {
        return new Promise<{ id: string; img: HTMLImageElement } | null>(
          (resolve) => {
